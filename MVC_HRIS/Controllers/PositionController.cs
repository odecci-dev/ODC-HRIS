...
namespace MVC_HRIS.Controllers
{
    public class PositionController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> GetPositionList( FilterPosition data)
        {
            string result = "";
            var list = new List<PositionPaginateModel>();
            try
            {
                HttpClient client = new HttpClient();
                var url = DBConn.HttpString + "/Position/PositionPaginationList";
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(token_.GetValue());
                StringContent content = new StringContent(JsonConvert.SerializeObject(data), Encoding.UTF8, "application/json");
                using (var response = await client.PostAsync(url, content))
                {
                    string res = await response.Content.ReadAsStringAsync();
                    list = JsonConvert.DeserializeObject<List<PositionPaginateModel>>(res);
